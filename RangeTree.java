...
public class RangeTree{
    ...
    public boolean hasConflict(Range query){
        // TODO
        Double start = query.start;
        Double end = query.end;

        Double nextEndKey = byEnd.findNextKey(start);
        if (nextEndKey != null) {
            Range nextRange = byEnd.find(nextEndKey);
            if (nextRange.start < end && nextRange.end > start) {
                return true;
            }
        } else {
            Double prevEndKey = byEnd.findPrevKey(end);
            if (prevEndKey != null) {
                Range prevRange = byEnd.find(prevEndKey);
                if (prevRange.start < end && prevRange.end > start) {
                    return true;
                }
            }
        }
